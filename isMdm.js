...
function isMdmMacOS () {
	...
	const command = spawnSync('/usr/bin/profiles', ['status', '-type', 'enrollment']).stdout.toString()

	if(command.includes('Enrolled via DEP: No') && command.includes('MDM enrollment: No')) {
		enrolled = false
	}
